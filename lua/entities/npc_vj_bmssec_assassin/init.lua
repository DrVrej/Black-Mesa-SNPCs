...
-----------------------------------------------*/
ENT.Model = {"models/VJ_BLACKMESA/hassassin.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want 
ENT.StartHealth = GetConVarNumber("vj_bms_assassin_h")
...
ENT.MeleeAttackDamage = GetConVarNumber("vj_bms_assassin_d")
...
ENT.NextMoveOrHideOnDamageByEnemy2 = 2.5 -- How much time until it moves or hides on damage by enemy? | The second # in math.random
...
function ENT:CustomOnInitialize()
	...
	if GetConVarNumber("vj_npc_noidleparticle") == 0 then
		...
		util.SpriteTrail(self,7,Color(200,0,0),true,6,6,0.1,1/(6+6)*0.5,"VJ_Base/sprites/vj_trial1.vmt")
	end
...
function ENT:CustomOnThink_AIEnabled()
	...
	if IsValid(self:GetEnemy()) && self.DoingWeaponAttack_Standing == true && self.VJ_IsBeingControlled == false && CurTime() > self.Assassin_NextJumpT && self:GetPos():Distance(self:GetEnemy():GetPos()) < 1400 then
		self:VJ_ACT_PLAYACTIVITY(VJ.PICK({ACT_LEAP,ACT_JUMP}),true,1,false)
